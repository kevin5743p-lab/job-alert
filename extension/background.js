...
import { buildPrompt, buildAnswersPrompt, buildFieldMapPrompt, buildFieldFillPrompt,
         ...
         cvFingerprint, DEFAULT_MODEL, MAX_TOKENS }
         from "./tailor_core.js";
// One tailoring, start to finish — shared with the apply engine, which makes
// its own packet when a job has never been tailored by hand.
...
//
// The tailoring itself lives in tailor_run.js, because the apply engine has to
// be able to do it too — a module cannot send itself a message and wait for the
// reply. This handler is now only the message surface: unwrap, call, wrap.
chrome.runtime.onMessage.addListener((msg, _sender, sendResponse) => {
  if (msg?.type !== "TAILOR") return;

  (async () => {
    try {
      // callGroq is passed in rather than imported over there: it is the
      // signed-out fallback, and every other piece of Groq plumbing — the key,
      // the quota handling, the shared JSON helper — is here.
      const out = await tailorJob(msg.job, {
        force: msg.force,
        groqTailor: callGroq,
      });
      sendResponse({
        ...
        saved: out.saved,
        signedIn: out.signedIn,
        reused: out.reused,
        tailoredAt: out.tailoredAt,
      });
    } catch (e) {
      sendResponse({ ok: false, error: String(e.message || e) });
    }
